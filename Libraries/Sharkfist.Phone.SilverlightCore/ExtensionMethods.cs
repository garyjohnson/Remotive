...
namespace Sharkfist.Phone.SilverlightCore
{
    ...
    {
        public static bool IsTrialOrTest(this LicenseInformation info)
        {
#if FREE
            return false;
#else
        ...
        {
            string workingString = hexString.Replace("#", string.Empty);
            byte a = System.Convert.ToByte(workingString.Substring(0, 2), 16);
            byte r = System.Convert.ToByte(workingString.Substring(2, 2), 16);
            byte g = System.Convert.ToByte(workingString.Substring(4, 2), 16);
            byte b = System.Convert.ToByte(workingString.Substring(6, 2), 16);
        ...
        public static T FindVisualChild<T>(this DependencyObject parent) where T : class
        {
            ...
            for (int i = 0; i < childCount; i++)
            {
                ...
                {
                    returnChild = child as T;
                    break;
                }
                else
                {
                    T innerChild = child.FindVisualChild<T>();
                    if (innerChild != null)
                    {
                        returnChild = innerChild;
                        break;
                    }
                }
            }
        ...
        public static double GetDistance(this Point origin, Point end)
        {
            return Math.Sqrt(Math.Pow(end.X - origin.X, 2.0d) + Math.Pow(end.Y - origin.Y, 2.0d));
        }
